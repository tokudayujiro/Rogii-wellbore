...
| 9 | 2026-06-30 | pfz_ensemble (hybrid + 0.5·CatBoost残差 + pfz特徴) | v10 | 7.890 (773 OOF) | **8.337** | — | exp013。第2トラッカー pf_z（Z速度連成）を残差特徴に。**LB ベスト**（pf_z で −0.27）。sub ref 54193403 |
| 10 | 2026-06-30 | tracker (hybrid + 0.6·(LGB+Cat)残差 + 3トラッカー不一致特徴) | v11 | **7.828** (773 OOF) | 8.414 | — | exp014。CV ベストだが LB は exp013 に僅かに劣後（軽い過適合）。sub ref 54195155 |

## 現状ベスト（CV と LB で分岐）

- **CV ベスト = exp014**（CV 7.828 / LB 8.414）— 773 坑井 OOF で最良、より信頼できる指標。
- **LB ベスト = exp013**（CV 7.890 / LB 8.337）— 隠し 3 坑井で最良、特徴少なめで汎化良好。

両者は僅差（CV 0.06 / LB 0.08 で逆方向）。**最終提出は Kaggle で 2 つ選べる**ので、
exp014（CV 重視）と exp013（LB 重視）の**両方を final に選択**するのが最適。
アクティブ kernel は v11(exp014)。carry_forward(12.81) から CV −39% / LB −46%。
CV 系譜: 12.81 → 8.18(PF) → 8.13(hybrid) → 7.97(rich) → 7.89(pfz) → **7.83(tracker)**。
LB 系譜: 15.88 → 8.90(PF) → 8.71(hybrid) → 8.61(stack) → 8.60(rich) → **8.34(pfz)**。
