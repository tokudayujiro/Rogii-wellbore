...
| 9 | 2026-06-30 | pfz_ensemble (hybrid + 0.5·CatBoost残差 + pfz特徴) | v10 | **7.890** (773 OOF) | 採点中 | — | exp013。第2トラッカー pf_z（Z速度連成）を残差特徴に。CV 全実験ベスト。sub ref 54193403 |

## 現状ベスト

**exp013 pfz_ensemble = CV 7.890**（kernel v10, `MODEL="combined"`, pfz_ensemble.joblib）。
carry_forward(12.81/15.88) から CV −38%。pf_z（pos=TVT・幾何dip速度prior・dual GR）を
残差モデルの特徴に加えて改善。参考 NB(LB 7.168) は複数トラッカー＋多段の幅で到達。
